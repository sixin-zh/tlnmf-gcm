# mle loss, a batch of samples, newton solver 
...
from .transform_learning_mleb import fast_transform_learning_mleb

def tl_nmf_batch(Ys, K, Phi=None, W=None, H=None, regul=None, max_iter=300,
    ...
    Parameters
    ----------
    Ys : array, shape (L, M, N)
        Frames matrix, M is the window size F
    ...
    Xs = np.matmul(Phi, Ys)
    Vs = Xs ** 2  # Initial spectrogram
    V = np.mean(Vs,axis=0)
    V_hat = np.dot(W, H)  # Initial factorization
    obj = new_is_div(V, V_hat, eps=eps_nmf) + regul * penalty(H, regul_type)  # Objective
    ...
    for n in range(max_iter):
        ...
        # Transform Learning
        V_hat = np.dot(W, H)
        obj1 = new_is_div(V, V_hat, eps=eps_nmf) + regul * penalty(H, regul_type)
        Phi_old = Phi.copy()
        Phi,Xs = fast_transform_learning_mleb(Phi, Xs, V_hat, n_iter_tl, eps=eps_nmf)
        Vs = Xs ** 2
        V = np.mean(Vs,axis=0)
        
        # Monitoring
        old_obj = obj.copy()
        obj = new_is_div(V, V_hat, eps=eps_nmf) + regul * penalty(H, regul_type)
