...
def analysis_windowing_per(signal, M, Nbox):
    t = np.linspace(0, np.pi, num=M).reshape(M, 1)
    w = np.sin(t)
    Xbig = np.concatenate((signal[-M//2:len(signal)], signal,
                           signal[0:M//2]))
    #Xbig = np.concatenate((np.zeros(M // 2), signal,
    #                       np.zeros(M // 2)), axis=0)
    #Xbig[0:M//2] = signal[-M//2-1:-1] # WRONG!
    #Xbig[-M//2-1:-1] = signal[0:M//2]
    
    #print('analysis_nowin_per Xbig',Xbig.shape)
...
def analysis_nowin_per(signal, M, Nbox):
    t = np.linspace(0, np.pi, num=M).reshape(M, 1)
    w = np.ones(t.shape)
    #print('append begin:',signal[-M//2:len(signal)])
    #print('append end:',signal[0:M//2])
...
def analysis_nowin(signal, M, Nbox):
    t = np.linspace(0, np.pi, num=M).reshape(M, 1)
    w = np.ones(t.shape)
    #print('w',w)
    Xbig = np.concatenate((np.zeros(M // 2), signal,
...
def signal_to_frames(signal, fs, window_size, fs_desired=None, nowin=False, per=False):
    ...
    if per:
        if nowin:
            return analysis_nowin_per(signal, n_window_samples, N_box)
        else:
            return analysis_windowing_per(signal, n_window_samples, N_box)
    else:
        # not sure why need zero padding here .. 
        padding = np.zeros(n_window_samples * N_box)
        signal = np.concatenate((signal, padding))
        if nowin:
            return analysis_nowin(signal, n_window_samples, N_box)
        else:
